      ...
      <SafeAreaView style={styles.container} edges={['bottom']}>
        ...
        <ScrollView contentContainerStyle={styles.scroll}>
          ...
          <View style={styles.specCard}>
            ...
            <Divider />
            <SpecRow label="Thread Depth/Height" v={nptDepth} fmt={(n) => fmtIn(n, 4)}
              alt={fmtMm(nptDepth * INCH_TO_MM, 3)} unit="in" altUnit="mm" highlight />
            <Divider />
      ...
      <SafeAreaView style={styles.container} edges={['bottom']}>
        ...
        <ScrollView contentContainerStyle={styles.scroll}>
          ...
          <View style={styles.specCard}>
            ...
            <Divider />
            <SpecRow label="Thread Depth/Height" v={bspDepth} fmt={(n) => fmtMm(n, 3)}
              alt={fmtIn(bspDepth / INCH_TO_MM, 4)} unit="mm" altUnit="in" highlight />
            {isTaper && (
              <>
                <Divider />
                <SpecRow label="Taper" v={0.0625} fmt={() => '1:16 (0.0625"/in)'}
...
function SegBtn({
  ...
  return (
    ...
    >
      {icon ? <Ionicons name={icon} size={16} color={active ? c.primary : c.muted} /> : null}
...
function makeStyles(c: AppColors) {
  return StyleSheet.create({
    ...
    title: { color: c.text, fontSize: 28, fontWeight: '800', fontFamily: 'monospace', marginTop: 6 },
    titleMeta: { flexDirection: 'row', gap: 12, marginTop: 14 },
    metaItem: {
      flex: 1, backgroundColor: c.surface, borderWidth: 1, borderColor: c.border,
      borderRadius: 4, paddingVertical: 10, paddingHorizontal: 12,
    },
    metaLabel: { color: c.muted, fontSize: 10, letterSpacing: 1, fontWeight: '700' },
    metaValue: { color: c.text, fontSize: 14, fontFamily: 'monospace', marginTop: 4, fontWeight: '700' },

    sectionLabel: {
      color: c.muted, fontSize: 12, fontWeight: '700', letterSpacing: 1.5,
      marginTop: 24, marginBottom: 10, textTransform: 'uppercase',
    },
    hint: { color: c.muted, fontSize: 12, marginTop: 8, fontStyle: 'italic' },

    segment: { flexDirection: 'row', gap: 10 },
    segmentSm: { flexDirection: 'row', gap: 8 },
    segBtn: {
      flex: 1, minHeight: 56, backgroundColor: c.surface,
    ...
    segLabel: { color: c.muted, fontWeight: '700', letterSpacing: 1, fontSize: 14 },
    ...
    specRowLabel: { color: c.muted, fontSize: 13, fontWeight: '600' },
    specRowValue: { color: c.text, fontSize: 17, fontFamily: 'monospace', fontWeight: '700' },
    specRowValueHi: { color: c.primary },
    specRowUnit: { color: c.dim, fontSize: 12 },
    specRowAlt: { color: c.dim, fontSize: 11, fontFamily: 'monospace', marginTop: 2 },
    ...
    basicLabel: { color: c.muted, fontSize: 10, letterSpacing: 1.2, fontWeight: '700' },
    basicValue: { color: c.text, fontSize: 18, fontFamily: 'monospace', fontWeight: '700', marginTop: 6 },
    basicUnit: { color: c.dim, fontSize: 11, marginTop: 2 },

    subLabel: { color: c.muted, fontSize: 11, letterSpacing: 1.2, fontWeight: '700', marginBottom: 8 },

    percentRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 6 },
    pctBtn: {
      flexBasis: '15%', flexGrow: 1, minHeight: 48, backgroundColor: c.bg,
      borderWidth: 1, borderColor: c.border, borderRadius: 4,
      alignItems: 'center', justifyContent: 'center',
    },
    pctBtnActive: { backgroundColor: c.primary, borderColor: c.primary },
    pctTxt: { color: c.muted, fontWeight: '800', fontSize: 14 },
    pctTxtActive: { color: c.onPrimary },

    resultBox: {
      marginTop: 18, backgroundColor: c.bg, borderLeftWidth: 3, borderLeftColor: c.primary,
      padding: 14, borderRadius: 4,
    },
    resultLabel: { color: c.muted, fontSize: 10, letterSpacing: 1.5, fontWeight: '700' },
    resultValue: { color: c.primary, fontSize: 32, fontFamily: 'monospace', fontWeight: '800', marginTop: 6 },
    resultUnit: { fontSize: 14, color: c.muted },
    resultAlt: { color: c.dim, fontSize: 13, fontFamily: 'monospace', marginTop: 4 },
    resultFooter: { color: c.dim, fontSize: 11, fontFamily: 'monospace', marginTop: 12 },

    closestRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 12 },
    closestText: { color: c.muted, fontSize: 12 },
    closestStrong: { color: c.success, fontWeight: '800', fontFamily: 'monospace' },

    formulaText: {
      color: c.muted, fontSize: 12, fontFamily: 'monospace', marginBottom: 14,
      backgroundColor: c.bg, padding: 10, borderRadius: 4,
    },
    wireRow: { flexDirection: 'row', gap: 12 },
    wireBlock: { flex: 1 },
    bestWire: { color: c.primary, fontSize: 22, fontFamily: 'monospace', fontWeight: '800' },
    bestWireAlt: { color: c.dim, fontSize: 11, fontFamily: 'monospace', marginTop: 2 },
    wireInput: {
      backgroundColor: c.bg, borderWidth: 1, borderColor: c.border,
      borderRadius: 4, paddingHorizontal: 12, height: 48, color: c.text,
      fontFamily: 'monospace', fontSize: 18, fontWeight: '700',
    },
    wireInputError: { borderColor: c.danger },
    rangeBox: {
      marginTop: 14, backgroundColor: c.bg, borderWidth: 1,
      borderColor: c.border, borderRadius: 4, padding: 12,
    },
    rangeRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 },
    rangeLabel: { color: c.muted, fontSize: 10, letterSpacing: 1.2, fontWeight: '700' },
    rangeFormula: { color: c.dim, fontSize: 11, fontFamily: 'monospace' },
    rangeValuesRow: { flexDirection: 'row', gap: 8 },
    rangeCol: {
      flex: 1, backgroundColor: c.surface, borderRadius: 4,
      paddingVertical: 8, paddingHorizontal: 6, alignItems: 'center',
    },
    rangeColLabel: { color: c.muted, fontSize: 9, letterSpacing: 1, fontWeight: '700' },
    rangeColValue: { color: c.text, fontSize: 14, fontFamily: 'monospace', fontWeight: '700', marginTop: 4 },
    rangeColAlt: { color: c.dim, fontSize: 10, fontFamily: 'monospace', marginTop: 1 },
    warnRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 10 },
    warnText: { color: c.danger, fontSize: 12, fontWeight: '600' },
    ...
    measureCol: { flex: 1 },
    measureMini: { color: c.muted, fontSize: 10, letterSpacing: 1, fontWeight: '700' },
    measureValue: { color: c.primary, fontSize: 22, fontFamily: 'monospace', fontWeight: '800', marginTop: 4 },
    measureAlt: { color: c.dim, fontSize: 11, fontFamily: 'monospace', marginTop: 2 },

    engRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 4 },
    engLabel: { color: c.text, fontSize: 13, fontWeight: '600', flex: 1 },
    engValue: { color: c.primary, fontSize: 18, fontFamily: 'monospace', fontWeight: '700' },
    engUnit: { color: c.dim, fontSize: 12 },
    engHint: { color: c.muted, fontSize: 11, fontStyle: 'italic', marginTop: 4, marginBottom: 6 },
  });
}
